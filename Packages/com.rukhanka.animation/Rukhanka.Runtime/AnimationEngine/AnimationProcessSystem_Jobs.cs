...
{
	unsafe void Execute(AnimatorParametersAspect apa, in DynamicBuffer<AnimationToProcessComponent> animationsToProcess)
	{
		if (animationsToProcess.IsEmpty) return;

		Span<float> layerWeights = stackalloc float[32];
		var numInt64ForParametersBitField = (int)math.ceil(apa.ParametersCount() / 64f);
		Span<ulong> bitFieldMemory = stackalloc ulong[numInt64ForParametersBitField];

		UnsafeBitArray isSetByCurve;
		fixed (void* ptr = &bitFieldMemory.GetPinnableReference())
		{
			isSetByCurve = new UnsafeBitArray(ptr, bitFieldMemory.Length * 8, Allocator.None);
		}
			
		Span<float> finalParamValues = stackalloc float[apa.ParametersCount()];
		Span<float> finalParamWeights = stackalloc float[apa.ParametersCount()];
		...
		for (int l = 0; l < animationsToProcess.Length; ++l)
		{
			...
			for (int k = 0; k < curves.Length && weight > 0; ++k)
			{
				...
				isSetByCurve.Set(paramIdx, true);
				...
				finalParamWeights[paramIdx] += weight;
		...
		{
			if (!isSetByCurve.IsSet(l)) continue;
			if (finalParamWeights[l] < 1)
				finalParamValues[l] += apa.GetParameterValueByIndex(l).floatValue * (1 - finalParamWeights[l]);
	...
	void ProcessEventsForAnimation(Entity e, ref DynamicBuffer<AnimationEventComponent> outEvents, ref BlobArray<AnimationEventBlob> events, float fStart, float fEnd, bool reverseEventIteration)
	{
		for (var i = 0; i < events.Length; ++i)
		{
			...
			{	
				...
				if (doDebugLogging)
					Debug.Log($"Emit event for entity {e}. Name: {ae.name.ToFixedString()}, F: {ae.floatParam}, I: {ae.intParam}, S: {ae.stringParam.ToFixedString()}");
			#endif
			}
		}
	}
