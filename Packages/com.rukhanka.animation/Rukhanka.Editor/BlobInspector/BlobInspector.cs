...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    void GatherAllBlobAssets(World world)
    {
        ...
        for (var i = 0; i < animControllersChunks.Length; ++i)
        {
            ...
            var bufAcc = chunk.GetBufferAccessor(controllerLayerBufHandle);
        ...
        for (var i = 0; i < smrChunks.Length; ++i)
        {
            ...
            var smrs = chunk.GetNativeArray(smrTypeHandle);
            var entities = chunk.GetNativeArray(entityHandle);
            for (var k = 0; k < chunk.Count; ++k)
            {
                var smr = smrs[k];
                ...
                if (!allBlobAssets.TryGetValue(smr.boneInfos.Value.hash, out var smrBlobInfo))
                {
                    smrBlobInfo = new BlobAssetInfo()
                    {
                        blobAsset = UnsafeUntypedBlobAssetReference.Create(smr.boneInfos),
                        blobType = BlobType.SkinnedMeshInfo,
                        refEntities = new ()
                    };
                    RegisterBlobAsset(smrBlobInfo, smr.boneInfos.Value.hash);
                }
        ...
        for (var i = 0; i < rigChunks.Length; ++i)
        {
            ...
            var rigs = chunk.GetNativeArray(rigDefTypeHandle);
            var entities = chunk.GetNativeArray(entityHandle);
            for (var k = 0; k < chunk.Count; ++k)
            {
                var rigDef = rigs[k];
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    string GatherBlobDBInfo()
    {
        ...
        var rv = $"Summary: {blobAssetsSummary.totalCount} blob assets, total memory {FormatMemory(blobAssetsSummary.sizeInBytes)}";
        return rv;
    }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    unsafe void CreateBlobAssetList()
    {
        ...
        for (var i = 0; i < (int)BlobType.Total; ++i)
        {
            groups[i].text = $"{countArr[i]} {foldoutLabels[i]} ({FormatMemory(memoryArr[i])})";
        }
    }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    unsafe void FillBlobAssetInfoText(TemplateContainer blobEntry, BlobAssetInfo ba)
    {
        ...
        #endif
            var infoText = $"References: {ba.refEntities.Count}, Size: {FormatMemory(ba.blobAsset.m_data.Header->Length)}";
        #if RUKHANKA_DEBUG_INFO
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    void FillBlobCacheInfo()
    {
        ...
        animMemLabel.text = $"Total animation blob cache size: {FormatMemory(animationsMem)}";
        controllerMemLabel.text = $"Total animator controller blob cache size: {FormatMemory(controllerMem)}";
    ...
    long FillBlobList(string foldoutName, string foldoutCaption, string cachePath)
    {
        ...
        if (Directory.Exists(cachePath))
        {
            ...
            for (var i = 0; i < files.Length; ++i)
            {
                ...
                sizeLabel.text = FormatMemory(fs);
                rv += fs;
